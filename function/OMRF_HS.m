function varargout = OMRF_HS(Img,probility2,beta,beta1,beta2,mra)
...
[subClass,GMMs] = CreateRgnSubclass(rgnData, object_k2, 6, 0.2);
...
theta_spec = 3;
spectral_weights = FeatureDisCalculating(Img,labels,statt,theta_spec);
...
% % model solution
%Low-semantic random field energy
tic
object_k1_img = OMRF(E_ob_k1+beta2*E_ClassInter_k1, labels, statt, region_weights, beta);
toc

%High-semantic random field energy
tic
object_k2_img = OMRF(E_ob_k2+beta2*E_ClassInter_k2, labels, statt, region_weights, beta1);
toc
